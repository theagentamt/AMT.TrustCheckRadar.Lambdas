...
def assert_active_device_binding(event: dict, account_id: str):
    ...
    LOGGER.info("Stage detail: device_binding_header_extracted")
    _require_table()
    LOGGER.info(
    ...
    if not active:
        LOGGER.warning("Stage detail: no_active_device_binding")
        raise AppError(
    ...
    if active_fingerprint != binding_fingerprint:
        LOGGER.warning("Stage detail: device_binding_mismatch")
        raise AppError(
            "DEVICE_BINDING_MISMATCH",
            "The active device binding for this account does not match the presented device.",
            retryable=False,
...
def _get_active_binding(account_id: str):
    LOGGER.info("Stage detail: device_binding_query_started")
    ...
    LOGGER.info("Stage detail: device_binding_query_completed | resultCount=%s", len(items))
