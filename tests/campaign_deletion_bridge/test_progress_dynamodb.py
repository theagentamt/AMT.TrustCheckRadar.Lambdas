...
def run(d,**kwargs):
    return service.delete_account_contributions(COMMAND,table_name='pipeline',deletion_ledger_table_name='ledger',
        retention_days=21,dynamodb=d,kms=SimpleNamespace(generate_mac=lambda **_:{'Mac':b'x'*32}),now_epoch=NOW,**kwargs)


def feature(put,n):
    for sk in ('FEATURE','DEDUPE','CLUSTERED'):
        row={'PK':f'EVENT#{n:03}','SK':sk,'expiresAt':NOW+1000}
        if sk=='FEATURE':row.update(GSI1PK=PART,GSI1SK=f'EVENT#{n:03}#FEATURE')
        put(row)


def candidate(put,count=61):
    put({'PK':'CANDIDATE#one','SK':'SUMMARY','version':1,'expiresAt':NOW+1000,'GSI3PK':'EXPIRY#dev',
         'GSI3SK':NOW+1000,'centroid':[Decimal('.9')],'contributorCount':999,'submissionCount':999})
    put({'PK':'CANDIDATE#one','SK':'CONTRIB#'+TOKEN,'GSI1PK':PART,'GSI1SK':'CANDIDATE#one',
         'submissionCount':1,'vectorApplied':True,'vector':[Decimal('.9')],'expiresAt':NOW+1000})
    for i in range(count):
        put({'PK':'CANDIDATE#one','SK':f'CONTRIB#other{i:03}','submissionCount':2,'vectorApplied':True,
...
def test_multiple_invocations_resume_every_index_row_without_receipt(world):
    ...
    for _ in range(40):
        ...
        assert result['complete'] is False and result['coverage']=='UNVERIFIED'
        if result['observedPassEnded']:
            saw_end=True;break
    assert saw_end
    for i in range(9):
        for sk in ('FEATURE','DEDUPE','CLUSTERED'):
            assert progress.get(d,'pipeline',f'EVENT#{i:03}',sk) is None
    assert progress.get(d,'ledger','ACCOUNT#a','ACCOUNT_DELETION#CAMPAIGN') is None
    tomb=progress.get(d,'pipeline',PART,'TOMBSTONE')
    assert tomb['expiresAt']==NOW+21*86400
...
def test_recompute_resumes_across_pages_without_partial_centroid(world):
    ...
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==999
    assert progress.get(d,'pipeline','CANDIDATE#one','DELETION_RECOMPUTE')['expiresAt']==NOW+1000
    run(d,max_steps=1)
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==999
    run(d,max_steps=1)
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==999
    run(d,max_steps=1)
    summary=progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')
    assert summary['contributorCount']==61 and summary['submissionCount']==122 and summary['centroid']==[Decimal('.5')]
    assert progress.get(d,'pipeline','CANDIDATE#one','DELETION_RECOMPUTE') is None
...
def test_lost_delete_acknowledgment_keeps_pending_recompute(world):
    ...
    assert progress.get(d,'pipeline',PART,'TOMBSTONE')['cleanupState']['phase']=='RECOMPUTE'
    assert progress.get(d,'pipeline','CANDIDATE#one','CONTRIB#'+TOKEN) is None
    run(d,max_steps=5)
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==1


def test_writer_version_change_restarts_recompute(world):
    d,put=world;candidate(put);run(d,max_steps=5)
    saved=progress.get(d,'pipeline','CANDIDATE#one','DELETION_RECOMPUTE');assert saved['contributors']==25
    d.update_item(TableName='pipeline',Key=progress.key('CANDIDATE#one','SUMMARY'),UpdateExpression='SET #v = :v',
                  ExpressionAttributeNames={'#v':'version'},ExpressionAttributeValues=progress.wire({':v':3}))
    run(d,max_steps=1)
    saved=progress.get(d,'pipeline','CANDIDATE#one','DELETION_RECOMPUTE')
...
def test_delayed_gsi_rows_after_empty_pass_do_not_create_completion(world):
    d,put=world
    assert run(d,max_steps=3)['observedPassEnded']
    feature(put,5)
    assert run(d,max_steps=5)['complete'] is False
    assert progress.get(d,'pipeline','EVENT#005','FEATURE') is None
    assert progress.get(d,'ledger','ACCOUNT#a','ACCOUNT_DELETION#CAMPAIGN') is None
...
def test_time_budget_returns_pending_without_work(world):
    ...
    assert progress.get(d,'pipeline','EVENT#001','FEATURE') is not None


def test_wrong_owned_base_row_fails_closed(world):
    d,put=world;feature(put,1);run(d,max_steps=2)
    d.update_item(TableName='pipeline',Key=progress.key('EVENT#001','FEATURE'),UpdateExpression='SET GSI1PK = :pk',
...
def test_new_operation_adopts_pending_recompute_without_reset(world):
    ...
    original=progress.get(d,'pipeline',PART,'TOMBSTONE')['cleanupState']
    ...
    progress.sweep(client,'pipeline',PART,other['operationId'],NOW,max_steps=1)
    adopted=progress.get(d,'pipeline',PART,'TOMBSTONE')['cleanupState']
    assert adopted==original|{'operationId':other['operationId']}
    progress.sweep(client,'pipeline',PART,other['operationId'],NOW,max_steps=4)
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==31
...
def test_overlapping_live_commands_preserve_shared_repair_and_deadline(world):
    ...
    progress.sweep(client,'pipeline',PART,withdrawal['operationId'],NOW,max_steps=2)
    run(d,max_steps=2) # re-adopt, continue second aggregate page
    assert progress.get(d,'pipeline','CANDIDATE#one','SUMMARY')['contributorCount']==31
    assert progress.get(d,'pipeline',PART,'TOMBSTONE')['expiresAt']==NOW+21*86400
    assert progress.get(d,'ledger','ACCOUNT#a','ACCOUNT_DELETION#CAMPAIGN') is None
...
def test_command_change_during_work_prevents_atomic_delete(world):
    ...
    with pytest.raises(d.exceptions.TransactionCanceledException):
        progress.sweep(client,'pipeline',PART,OP,NOW,max_steps=1)
    assert progress.get(d,'pipeline','EVENT#001','FEATURE') is not None
