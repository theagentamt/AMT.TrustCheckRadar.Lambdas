...
For a `prepared` envelope, `expiresAt` is the deadline to admit the operation; it is not a deadline to reconcile an already-admitted check. The backend accepts an expired execution proof for authenticated, same-account, URL-free reconciliation while the receipt remains inside its retention deadline. For a settled envelope, `expiresAt` is the receipt expiry when available. Missing/expired receipts return unknown accounting, never an assertion that no charge occurred. The owner approved seven-day minimized receipts on 2026-09-20. Clients may retain only encrypted account-bound check identity/proof and timestamps for at most seven days. Logout or account switch clears URLs, results and current UI and prohibits other-account access; the minimized proof remains available only for same-account reauthentication and accounting recovery until expiry. Confirmed account deletion erases the proof. Never retain the original URL in this recovery record. This clarification does not change schema version or activate a route.
