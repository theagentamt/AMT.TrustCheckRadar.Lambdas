...
Candidate.2 narrows purchases to `platform`, `productId`, `verifiedAtEpoch` from the
new verified ownership projection. Subscription periods/status remain in their own
family. The included manifest list is a scope set, not traversal order: authority
families may repeat per retained namespace. At least one page (possibly empty) is
emitted for every included family. A client must observe every family before it
accepts COMPLETE. Neither a fixed final-family name nor manifest ordering is a
client assumption. Isolated first/final page fixtures test envelope decoding; they
do not alone constitute a complete assembled export sequence.

Candidate.3 adds play_verification metadata only under ACCOUNT_EXPORT_PLAY_TOKENS_ENABLED. No purchase token, ciphertext, wrapped key, token digest, binding hash or KMS identity is exported. Pending-token transport copies remain disclosed under excluded transient streams/queues. Candidate.2 files are unchanged. Old clients must reject unknown candidate.3 until their explicit integration.
