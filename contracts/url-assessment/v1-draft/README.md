...
- `target.url` is an eligible, reviewed HTTP/HTTPS request URL, excluding its fragment. `target.scope=origin_only` explicitly declares reduced input and `withheldComponents` names what was removed; the original values/replacement map are never transmitted. Do not silently rewrite a full URL and claim to have checked it. Client flags are untrusted and require independent API validation. Full-value submission rules remain a privacy gate; schema validity alone authorizes no network request.
