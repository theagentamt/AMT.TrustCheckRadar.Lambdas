#!/usr/bin/env python3
"""Build evidence and publish only the thirteen reviewed Play lifecycle Dev packages.
...
FUNCTIONS = ('play_lifecycle_ingress', 'play_lifecycle_worker', 'play_token_deletion', 'v1_play_handoff', 'url_consumer', 'url_lease_recovery', 'v1_entitlements', 'v1_authority_deletion', 'message_consumer', 'recovery_consumer', 'result_feedback', 'account_export_api', 'account_data_api')
...
def inspect_packages(dist, source_sha):
    ...
    if {p.name for p in dist.glob('*.zip')} != expected:
        raise ValueError('Exactly the thirteen Play lifecycle packages are required')
    entries = {}
    ...
    for function in FUNCTIONS:
        ...
        artifacts.append({'function': function, 'artifact': path.name,
                          ...
                          'handler': (function+'.app.lambda_handler' if function in ('v1_play_handoff','v1_entitlements','v1_authority_deletion') else 'app.lambda_handler'), 'bundledDependencies': dependencies})
    return {'schemaVersion': 1, 'scope': 'play_lifecycle_disabled_candidate', 'sourceSha': source_sha,
