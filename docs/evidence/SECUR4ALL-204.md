...
| Random retry-stable event identity | A UUIDv4 is stored with `RESULT_READY` and reused by recovery before the atomic outbox commit. | `test_result_ready_is_replayed_for_atomic_commit_recovery`, service recovery assertions, publisher UUIDv4 contract tests |
