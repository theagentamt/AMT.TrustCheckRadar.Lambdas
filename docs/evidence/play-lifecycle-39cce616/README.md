...
## Closed privacy candidate installation

After the eight-function installation above, the orchestrator installed the existing `account_export_api` and `account_data_api` artifacts from the same reviewed source and exact published S3 versions. `aws-privacy-disabled-smoke.json` records two authorized empty-event AWS invocations at 2026-09-23T06:14:23Z. These existing Terraform resources use unqualified `$LATEST`; they do not have published `live` aliases.

The harness required the supplied configuration revision IDs, published code hashes, Python 3.14/arm64, exact handlers and closed export/token-export/deletion/identity gates before either invocation. It rechecked configuration immediately before and after each invocation. Both returned HTTP 503 with the expected code (`SERVICE_NOT_ENABLED` for export and `FEATURE_DISABLED` for account-data), `$LATEST` execution and no `FunctionError`. No revision changed. An unqualified Lambda Invoke has no atomic RevisionId precondition; pre/post checks are a practical drift check, not immutable alias pinning.

Only `{}` was sent, once per function; no logs were requested and SDK retries were limited to one attempt. The reviewed disabled branches return before account/provider operations. No feature, inventory, identity mapping, cursor secret or deletion policy was activated or approved by these checks. Enabled exports, deletion, token erasure, finalization and actual retention deadlines remain separate acceptance. Infrastructure configuration, IAM and no-drift results are separately recorded by the orchestrator.

This brings the installed source-aligned subset to ten of the thirteen published artifacts. `message_consumer`, `recovery_consumer` and `result_feedback` remain undeployed candidates in this rollout. The earlier five-candidate statement describes the preceding eight-function installation. No source or archive rebuild was needed.
