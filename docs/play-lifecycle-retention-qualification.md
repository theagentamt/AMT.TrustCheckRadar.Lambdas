...
| Account deletion | Deploy the source-aligned account-data/finalizer candidate, requalify the whole-account inventory to require `PLAY_TOKENS`, and qualify actual stream/catch-up composition and component ordering. This test supplies other component receipts; it does not certify their cleanup. |
| Export | Deploy the optional source-aligned export candidate with exact read-only token-table permissions and enable candidate.3 only for explicitly compatible consumers. Authenticated live export/deletion race and full retained-family coverage remain to be qualified. |
...
The existing eight closed Dev functions need no rebuild for this test/docs-only change. Five other published archives remain candidate artifacts rather than deployed functions. SECUR4ALL-125 remains In Progress; this increment closes only the stated synthetic qualification gap.
