...
## Transient observation recovery

A later actual designated deletion completed with all twelve receipts, Cognito absence and all 37 baseline unrelated rows preserved. Its token monitor stopped after 92 polls on HTTP 503 without an application error code. That response does not establish a concurrency cause or an erasure failure, and the discarded token cannot be recovered. The original accepted operation must not be reset or resubmitted.

The operator monitor now retries only HTTP 429/500/502/503/504 and HTTP transport failures within the existing maximum of 600 seconds and 120 polls, with five seconds between attempts. It records bounded per-status retry counts, transport failure count and allowlisted last status/error code, without response bodies, messages or tokens. HTTP 500 remains a visible failure count even if a later observation succeeds. An empty or non-JSON retryable gateway response supplies only its status to this loop. Other malformed or unexpected responses still stop observation. Cognito errors retain the existing fail-closed classification.

Each poll resets both denial flags; retryable failures do not call Cognito or count as application denial. Fresh time/budget checks after each network operation prevent natural expiry from proving rejection. A permanent failure exhausts the observation budget while preserving the accepted journal. This is an operator-only correction with no Lambda, concurrency, schedule, credential-retention or request-retry change. A separately authorized fresh synthetic acceptance would be required to obtain new token evidence; no prior operation is reused.

Validation for this increment: 22 monitor tests pass, including every retryable status followed by a same-poll dual denial, permanent 503 time exhaustion, the 120-attempt transport cap, transient transport recovery, expiry during failed network calls, and empty/non-JSON gateway responses. These synthetic tests do not claim actual new HTTP or token qualification.
