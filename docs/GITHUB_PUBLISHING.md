...
The `Publish Lambda release` workflow promotes the exact ZIP packages and ARM64
feature image produced by a successful `CI` push run on `main`. It does not
rebuild deployment artifacts during promotion. The source commit SHA is the
immutable release ID used in S3 and as the ECR image tag.
...
| `FEATURE_IMAGE_ECR_REPOSITORY` | No | Environment ECR repository name for the campaign feature-extractor image |
...
- `s3:PutObject` for `releases/*` in that bucket; and
- when the feature image is enabled, ECR login, layer upload, image publication,
  and `ecr:DescribeImages` for its configured repository.
...
2. Dev publishes automatically when `ENABLE_DEV_PUBLISH=true`. The workflow
   summary records the source CI run ID, release SHA, S3 prefix, and optional ECR
   digest.
3. To promote the identical ZIP bytes and feature image, run `Publish Lambda
   release` manually from `main`, choose `uat` or `prod`, and enter the successful
   source CI run ID.
4. Pass the recorded release SHA and optional feature-image digest to the
