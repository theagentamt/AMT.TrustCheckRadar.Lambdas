...
| `SECUR4ALL-207` | Process consent withdrawal/account deletion, remove active contributions, recompute affected candidates, sweep retention, and retire period keys. | Withdrawal, contribution/feature/dedupe erasure, recomputation, key retirement, and component receipts are implemented. A disabled atomic account-deletion producer/status path, post-fence session revocation, reconciliation, and bounded device cleanup now exist. Completion remains blocked by the complete data inventory, final identity/overall finalizer and fence-retention decisions, the missing campaign expiration query access pattern, and deployed retention/backup/DLQ/UAT evidence. |
