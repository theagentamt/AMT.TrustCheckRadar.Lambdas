...
| `SECUR4ALL-207` | Process consent withdrawal/account deletion, remove active contributions, recompute affected candidates, sweep retention, and retire period keys. | Strong locator traversal, bounded centroid/count repair and publication recovery exist as disabled candidates. The new read-only retained-period coverage assessor diagnoses incomplete evidence; campaign/withdrawal completion helpers remain hard-disabled. Actual receipt production still requires derived metadata repair, full legacy/period/key coverage, retirement ordering, durable reconciliation and deployed retention/restore/deadline evidence. See campaign-completion-coverage-assessor.md; historical implementation claims are not current completion proof. |
